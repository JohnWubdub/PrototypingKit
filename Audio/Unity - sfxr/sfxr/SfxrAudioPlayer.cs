...
public class SfxrAudioPlayer : MonoBehaviour {
   ...
   void Start() {
#if UNITY_WEBGL
			// WebGL doesn't allow OnAudioFilterRead, or any other parallel generatio  n really. So we generate it completely once the sound starts (will still be cached).
			runAsAudioClip = true;
#endif
      ...
      {
         masterMixer = AudioDirector.Instance.Mixer;
         AudioMixerGroup[] mixers = masterMixer.FindMatchingGroups("SFXR");
         soundSource.outputAudioMixerGroup = mixers[0];
      }
      if (waitBeforePlay){
   ...
   public void SetPlayTime(double _time) {
      waitBeforePlay = true;
      startTime = _time;
   }
